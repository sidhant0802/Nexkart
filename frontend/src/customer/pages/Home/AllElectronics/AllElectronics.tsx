...
const AllElectronics = () => {
  ...
  const dbItems               = homePage?.homePageData?.electronicsViewAll || [];
    ...
    <div className={`min-h-screen ${isDark ? "bg-[#0a0a0f]" : "bg-gray-50"}`}>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div>
              ...
              <p className={isDark ? "text-white/40" : "text-gray-600"}>
                {total} products across {Object.keys(grouped).length} categories
              </p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-12 space-y-12">
        {Object.entries(grouped).map(([subName, subItems], gIdx) => {
          ...
          const color = COLORS[subName] || "#6366f1";
          ...
          return (
            <motion.div key={subName} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className={`flex items-center justify-between p-5 rounded-2xl border mb-6`}
                style={{
                  background: isDark ? `${color}10` : `${color}10`,
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {items.map((item: any, i: number) => (
                  <motion.div key={item._id || item.name + i}
                    ...
                    <Link to={`/products/${item.categoryId}`}>
                      <motion.div whileHover={{ y: -6, scale: 1.04 }} whileTap={{ scale: 0.96 }}
                        ...
                        {/* Discount badge */}
                        {item.discount && (
                          <span className="absolute -top-2 -right-2 z-10 px-2 py-0.5 rounded-md text-[10px] font-black text-white"
                        ...
                        <div className="flex items-center gap-0.5 text-[10px] font-bold uppercase opacity-0 group-hover:opacity-100 transition-opacity"
                          style={{ color }}>
        ...
        {total === 0 && (
          <div className="text-center py-20">
            <p className={isDark ? "text-white/40" : "text-gray-400"}>No items found</p>
          </div>
        )}
      </div>
    </div>
