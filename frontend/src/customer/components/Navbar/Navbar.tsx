// src/customer/components/Navbar/Navbar.tsx
...
import {
  ...
  Package, ChevronDown, Zap, Bell, X,
...
import NavbarSearch from "../Search/NavbarSearch";   // ✅ ADDED
...
const Navbar = () => {
  ...
  const avatarGrad   = isAdmin  ? "linear-gradient(135deg, #ef4444, #dc2626)"
                     : isSeller ? "linear-gradient(135deg, #10b981, #059669)"
                                : "linear-gradient(135deg, #8b5cf6, #6366f1)";
  const avatarShadow = isAdmin  ? "0 4px 14px rgba(239,68,68,0.45)"
                     : isSeller ? "0 4px 14px rgba(16,185,129,0.45)"
  ...
  return (
    <>
          ...
          >
            <div className="max-w-[1400px] mx-auto px-4 lg:px-8 h-9 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4 text-xs">
                ...
                <Link to="/account/orders" className="text-white/50 hover:text-white transition-colors">
                  Track Order
                </Link>
              </div>
            </div>
      ...
      >
            ...
            <Link to="/" className="flex-shrink-0">
              <motion.div whileHover={{ scale: 1.03 }} className="flex items-center gap-2.5">
                <div className="relative">
                  <div className="absolute inset-0 rounded-xl bg-purple-500/40 blur-md" />
                  <div className="relative w-9 h-9 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center">
                    <Zap size={18} className="text-white" fill="white" />
                  </div>
                </div>
                <span
                  className="logo text-xl font-black tracking-tight hidden sm:block"
                  style={{
            ...
            <div className="flex items-center gap-1.5 ml-auto flex-shrink-0">
              ...
              >
                <AnimatePresence mode="wait">
                  {isDark ? (
                    <motion.div key="sun" initial={{ rotate: -90, opacity: 0, scale: 0.5 }}
                      animate={{ rotate: 0, opacity: 1, scale: 1 }}
                      exit={{ rotate: 90, opacity: 0, scale: 0.5 }} transition={{ duration: 0.2 }}>
                      <Sun size={17} />
                    </motion.div>
                  ) : (
                    <motion.div key="moon" initial={{ rotate: 90, opacity: 0, scale: 0.5 }}
                      animate={{ rotate: 0, opacity: 1, scale: 1 }}
                      exit={{ rotate: -90, opacity: 0, scale: 0.5 }} transition={{ duration: 0.2 }}>
                      <Moon size={17} />
                    </motion.div>
              ...
              <Link to={isLoggedIn ? "/wishlist" : "/login"}>
                ...
                >
                  <Heart size={17} className="group-hover:text-pink-400 transition-colors" />
                  {wishCount > 0 && (
              ...
              >
                <ShoppingCart size={17} className="group-hover:text-indigo-400 transition-colors" />
                {cartCount > 0 && (
                  <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }}
              ...
              {/* Account / Login - ROLE AWARE DROPDOWN */}
              {isLoggedIn ? (
                <div className="relative hidden sm:block ml-3">
                  <motion.button
                    ...
                    className={`flex items-center gap-2 pl-1 pr-3 py-1 rounded-full transition-all ${
                      isDark ? "bg-white/6 hover:bg-white/10"
                  ...
                  >
                    ...
                    <span className={`text-sm font-medium max-w-[80px] truncate ${isDark ? "text-white/80" : "text-gray-700"}`}>
                      {displayName?.split(" ")[0] || "Me"}
                    </span>
                    <ChevronDown size={12}
                      className={`transition-transform duration-200 ${userMenuOpen ? "rotate-180" : ""} ${isDark ? "text-white/30" : "text-gray-400"}`}
                    />
                  </motion.button>
                    ...
                    {userMenuOpen && (
                      <>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3 relative z-10">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 mt-1.5 rounded-full text-[10px] font-bold bg-white/20 text-white">
                                  {isAdmin ? <Shield size={9} /> : isSeller ? <Store size={9} /> : <User size={9} />}
                                  {displayBadge}
                                </span>
                              </div>
                            </div>
                          ...
                          {(isSeller || isAdmin) && (
                            <div className="p-3 border-b" style={{ borderColor: isDark ? "rgba(255,255,255,0.06)" : "#f3f4f6" }}>
                              <Link
                          ...
                          {isCustomer && (
                            <div className="p-3 border-b" style={{ borderColor: isDark ? "rgba(255,255,255,0.06)" : "#f3f4f6" }}>
                              <Link to="/become-seller" onClick={() => setUserMenuOpen(false)}>
                                <motion.div
                                  whileHover={{ scale: 1.02 }}
                                  whileTap={{ scale: 0.98 }}
                                  className="flex items-center justify-between p-3 rounded-xl"
                                ...
                                >
                                  <div className="flex items-center gap-2.5">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-white/75 text-[10px] leading-tight mt-0.5">Start selling on Nexkart</p>
                                    </div>
                                  </div>
                                  <ArrowRight size={16} className="text-white" />
                                </motion.div>
                          ...
                          <div className="p-2">
                                  ...
                                  <Link key={item.label} to={item.href} onClick={() => setUserMenuOpen(false)}>
                                    <motion.div whileHover={{ x: 4 }}
                                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${
                                        isDark ? "text-white/60 hover:text-white hover:bg-white/6"
                            ...
                            {isSeller && !isAdmin && (
                              <>
                                {[
                                  ...
                                  { icon: User,    label: "Seller Profile", href: "/seller/account" },
                                ].map((item) => (
                                  <Link key={item.label} to={item.href} onClick={() => setUserMenuOpen(false)}>
                                    <motion.div whileHover={{ x: 4 }}
                                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${
                            ...
                            <div className={`my-1.5 h-px ${isDark ? "bg-white/5" : "bg-gray-100"}`} />
              ...
              <motion.button
                ...
                className={`lg:hidden w-9 h-9 flex items-center justify-center rounded-xl ml-1 transition-all ${
                  isDark ? "text-white/50 hover:text-white hover:bg-white/8"
        ...
        {/* ✅ Mobile Search Bar — using NavbarSearch */}
        <AnimatePresence>
          {mobileSearch && (
            <motion.div
                ...
                <div className="flex items-center h-11 gap-1">
                  ...
                  {mainCategory.map((cat) => (
                    ...
                    >
                      ...
                      >
                        {cat.name}
                        <motion.div animate={{ rotate: activeCat === cat.categoryId ? 180 : 0 }} transition={{ duration: 0.2 }}>
        ...
        <AnimatePresence>
          {activeCat && (
            <motion.div
              ...
              style={{
                boxShadow: isDark ? "0 20px 60px rgba(0,0,0,0.5)" : "0 20px 40px rgba(0,0,0,0.1)",
              }}
