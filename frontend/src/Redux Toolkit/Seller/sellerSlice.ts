...
export const fetchSellerProfile = createAsyncThunk<Seller, any>(
  ...
  async (jwt: string, { rejectWithValue }) => {
    try {
      ...
      console.log("✅ seller profile loaded:", response.data?.email);
      return response.data;
    } catch (error: any) {
      // ✅ Silent for 404 (means user is a customer, not seller — expected)
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        return rejectWithValue("NOT_A_SELLER");
      }
      // Only log unexpected errors
      console.warn("⚠️ Seller profile fetch failed:", error.response?.status);
...
>("sellers/fetchSellerReport", async (jwt: string, { rejectWithValue }) => {
  try {
const response = await api.get(`/api/sellers/report`, {
      headers: {
